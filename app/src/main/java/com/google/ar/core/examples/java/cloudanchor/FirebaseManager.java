...
class FirebaseManager {
  ...
  /**
   * Gets the last room code from the Firebase Database. Invokes the listener with the answer.
   */
  void getLastRoomCode(RoomCodeListener listener) {
    Preconditions.checkNotNull(app, "Firebase App was null");
    roomCodeRef.runTransaction(
            new Transaction.Handler() {
              @Override
              public Transaction.Result doTransaction(MutableData currentData) {
                ...
                Object currVal = currentData.getValue();
                if (currVal != null) {
                  lastCode = Long.valueOf(currVal.toString());
                }
                currentData.setValue(lastCode);
                return Transaction.success(currentData);
              }
